...
import {
  CalendarDays, // For overall page title
  ...
  Loader2, // For loading application
} from "lucide-react";

const ScheduleInterview = () => {
  const { applicationId } = useParams();
  const [application, setApplication] = useState(null);
  const [loadingApp, setLoadingApp] = useState(true); // Loading state for application details
  const [scheduling, setScheduling] = useState(false); // Loading state for scheduling submission
    ...
    const fetchApplication = async () => {
      ...
      try {
        const res = await axios.get(
          `http://localhost:5000/api/applications/${applicationId}`, // Assuming /api/applications/:id route exists
          { headers: { Authorization: `Bearer ${token}` } }
        );
        // Ensure your backend `getApplicationById` populates the 'job' field
        setApplication(res.data.application);

        // Pre-fill date with a sensible default, e.g., 3 days from now
  ...
  const handleSubmit = async (e) => {
    ...
    // Ensure application data is loaded before proceeding
    if (!application) {
    ...
    try {
      await axios.post(
        `http://localhost:5000/api/interviews`, // This is the /api/interviews POST route
        {
          application: applicationId, // Use 'application' instead of 'applicationId' if your backend expects the ref name
          job: application.job._id, // ⭐ IMPORTANT: Pass job ID from populated application ⭐
          applicant: application.user._id, // ⭐ IMPORTANT: Pass applicant ID from populated application ⭐
          recruiter: application.job.recruiter, // ⭐ IMPORTANT: Pass recruiter ID from populated job ⭐
          date: form.date,
          mode: form.mode,
          location: form.location,
          notes: form.notes,
          jobTitle: application.job.title // ⭐ Pass job title for easier tracking on interview side ⭐
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success("Interview scheduled successfully!");

      // ⭐ CRITICAL CHANGE: Navigate back to the specific JobApplicantsList page ⭐
      ...
      toast.error(err.response?.data?.error || "Failed to schedule interview. Please check inputs and try again.");
    } finally {
      setScheduling(false);
    }
  };
  ...
  if (loadingApp) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex justify-center items-center">
        ...
        <p className="ml-4 text-xl font-medium text-gray-700">Loading application details...</p>
      </div>
    );
  }

  // If application is null after loading, it means it wasn't found or an error occurred
  if (!application) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col justify-center items-center p-8">
        <div className="flex flex-col items-center justify-center text-gray-600 bg-white p-12 rounded-2xl shadow-lg max-w-lg mx-auto border border-gray-200">
          <CalendarDays className="w-16 h-16 text-red-400 mb-6 animate-pulse-slow" />
          <p className="text-xl font-medium text-gray-700 mb-2">Application Not Found!</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-10 px-4 sm:px-6 lg:px-8 flex items-center justify-center animate-fade-in">
      <div className="bg-white p-8 sm:p-10 rounded-2xl shadow-xl w-full max-w-xl border border-gray-200 transform transition-all duration-300 hover:shadow-2xl">
        <h2 className="text-3xl font-extrabold text-center text-blue-800 mb-8 tracking-tight">
          <CalendarDays className="inline-block w-8 h-8 mr-3 text-indigo-600" /> Schedule Interview
        </h2>

        {/* Applicant and Job Info */}
        <div className="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-200">
          <p className="text-lg font-semibold text-blue-800 mb-2 flex items-center">
            ...
            Applicant: <span className="ml-2 text-gray-800">{application.user?.name}</span> {/* Use application.user.name as per getApplicationById */}
          </p>
          <p className="text-lg font-semibold text-blue-800 flex items-center">
            <BriefcaseBusiness className="w-6 h-6 mr-3 text-blue-600" />
            Job Title: <span className="ml-2 text-gray-800">{application.job?.title}</span> {/* Use optional chaining for job.title */}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {scheduling ? (
              ...
              <>
                <Send size={20} className="mr-2" /> Schedule Interview
              </>
            )}
          </button>
        </form>
